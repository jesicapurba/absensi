...
<div class="bg-white rounded-lg shadow-md p-6">
    ...
    <form action="{{ route('presensi.index') }}" method="GET">
        <input type="text" name="search" placeholder="Cari Absen..." value="{{ request('search') }}">
    ...
    <tbody>
    @forelse($presensi as $index => $k)
        <tr class="border-b-2 border-gray-400 hover:bg-gray-50">
            <td>{{ $loop->iteration + ($siswa->currentPage() - 1) * $presensi->perPage() }}</td>
            <td class="py-3 px-4">{{ $k->name }}</td>
            <td class="py-3 px-4">{{ $k->datang }}</td>
            <td class="py-3 px-4">{{ $k->pulang }}</td>
            <td class="py-3 px-4">
            <a href="{{ route('presensi.create', ['user_id' => $k->id]) }}" class="btn btn-primary">
                <i class="bi bi-plus-circle"></i>Presensi
            </a>

            <a href="{{ route('presensi.show', $k->id) }}" class="btn btn-sm btn-info" title="Lihat Detail">
                <i class="fas fa-eye">Lihat Details</i>
            </a>
            </td>
            <td class="py-3 px-4">
                <div class="flex justify-center space-x-2">
                    <a href="{{ route('presensi.edit', $k->id) }}" class="text-green-500 hover:text-green-700">
                        <i class="fas fa-edit">Edit</i>
                    </a>
                <form action="{{ route('presensi.destroy', $k->id) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
    ...
    @empty
    <tr>
        <td colspan="9" class="py-4 px-4 text-center">Tidak ada presensi siswa.</td>
    </tr>
